...
class REPORTMODEL extends APIRESPONSE
{
  ...
  public function generateReport($data, $loginData, $type)
  {
    ...
    try {
      ...
      $saleQuery = "SELECT id, invoice_number,sale_date, customer_name, city, product_type, product_quantity, unit, wages, transport, amount,payment_status FROM tbl_sales WHERE status = 1 and created_by = " . $loginData['id'] . $saleDateWiseFilter . $paymetWise . $customerNameWise;
      // echo $saleQuery;
      ...
      $saleAmountQuery = "SELECT ROUND(SUM(wages+transport+amount), 2) as amount FROM tbl_sales WHERE status = 1 and created_by = " . $loginData['id'] . $saleDateWiseFilter . $paymetWise . $customerNameWise;
      $saleAmount = $this->totalAmountCalculation($saleAmountQuery);
      $sale_amount = $saleAmount['amount'];
      $saleQuantityQuery = "SELECT SUM(product_quantity) as product_quantity  FROM tbl_sales WHERE status = 1 and created_by = " . $loginData['id'] . $saleDateWiseFilter . $paymetWise . $customerNameWise;
      $sale_Quantity = $this->totalAmountCalculation($saleQuantityQuery);

      $purchaseQuery = "SELECT id,invoice_number,purchase_date, vendor_name, city, product_type, product_quantity, unit, wages, transport, amount,payment_status FROM tbl_purchase WHERE status = 1 and created_by = " . $loginData['id'] . $purchaseDateWiseFilter . $paymetWise . $vendorNameWise;
      ...
      $purchaseQuantityQuery = "SELECT SUM(product_quantity) as product_quantity  FROM tbl_purchase WHERE status = 1 and created_by = " . $loginData['id'] . $purchaseDateWiseFilter . $paymetWise . $vendorNameWise;
      $purchase_Quantity = $this->totalAmountCalculation($purchaseQuantityQuery);

      $purchaseAmountQuery = "SELECT ROUND(SUM(wages+transport+amount), 2) as amount FROM tbl_purchase WHERE status = 1 and created_by = " . $loginData['id'] . $purchaseDateWiseFilter . $paymetWise . $vendorNameWise;
      $purchaseAmount = $this->totalAmountCalculation($purchaseAmountQuery);
      $purchase_amount = $purchaseAmount['amount'];
      if ($paymetWise == '') {
        $lossQuery = "SELECT id,invoice_number,loss_date, vendor_name, city, product_type, product_quantity, unit, amount FROM tbl_loss WHERE status = 1 and created_by = " . $loginData['id'] . $lossDateWiseFilter . $vendorNameWise;
        ...
        $lossAmountQuery = "SELECT ROUND(SUM(amount), 2) as amount FROM tbl_loss WHERE status = 1 and created_by = " . $loginData['id'] . $lossDateWiseFilter . $vendorNameWise;
      ...
      if ($type == "generate") {
        ...
        $this->generateExcelReport($sale_data, $purchase_data, $loss_amount, $totalIncome, $profit, $expenses, $total_sale_product_quantity, $total_purchase_product_quantity, $paymetWise, $loss_data);
  ...
  public function generateExcelReport($sale_data, $purchase_data, $loss_amount, $totalIncome, $profit, $expenses, $total_sale_product_quantity, $total_purchase_product_quantity, $paymetWise, $loss_data)
  {
    $dateNow = date("Y-m-d");
        ...
        <thead>
          <tr>
            ...
            <th width="25%">
              ...
              <div>' . $total_purchase_product_quantity . '</div>
            </th>
          </tr>
        </thead>
      </table>
